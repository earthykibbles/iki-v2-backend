...
from meals.schemas import NutritionPlan, EnhancedProfile, MealPlan, FoodIdentity, FoodIngredients, FoodRecipe
import logging
...
def process_image(image_data, keyword):
    ...
    if keyword == "foodid":
        mdu = FoodIdentity
        prompt = ("What is this foods name? "
                  "(Your search should be first Kenyan as I am a Kenyan before giving me international names.)"
                  "Notice if the image is a picture or a real food scan by a user. If it is a picture add (Picture) at end of food name.")
