...
from api.tasks import (
    generate_content_task, generate_fitness_landing_task,
    generate_mindfulness_landing_task, generate_nutrition_landing_task
)
...
class GenerateContentView(APIView):
    ...
    def post(self, request):
        ...
        if not prompt or not schema:
            return Response({'status': 'error', 'message': 'Prompt and schema are required'}, status=400)

        task = generate_content_task.delay(prompt, schema)
...
class GenerateContentStatusView(APIView):
    ...
    def get(self, request, task_id):
        task_result = generate_content_task.AsyncResult(task_id)
...
class GenerateFitnessLandingView(APIView):
    def get(self, request):
        task = generate_fitness_landing_task.delay()
        return Response({'task_id': task.id, 'status': 'pending'}, status=status.HTTP_202_ACCEPTED)

class GenerateFitnessLandingStatusView(APIView):
    def get(self, request, task_id):
        task_result = generate_fitness_landing_task.AsyncResult(task_id)
        if task_result.ready():
            result = task_result.result
            if result['status'] == 'completed':
                return Response(result['result'], status=status.HTTP_200_OK)
...
class GenerateMindfulnessLandingView(APIView):
    def get(self, request):
        task = generate_mindfulness_landing_task.delay()
        return Response({'task_id': task.id, 'status': 'pending'}, status=status.HTTP_202_ACCEPTED)

class GenerateMindfulnessLandingStatusView(APIView):
    def get(self, request, task_id):
        task_result = generate_mindfulness_landing_task.AsyncResult(task_id)
        if task_result.ready():
            result = task_result.result
            if result['status'] == 'completed':
                return Response(result['result'], status=status.HTTP_200_OK)
...
class GenerateNutritionLandingView(APIView):
    def get(self, request):
        task = generate_nutrition_landing_task.delay()
        return Response({'task_id': task.id, 'status': 'pending'}, status=status.HTTP_202_ACCEPTED)

class GenerateNutritionLandingStatusView(APIView):
    def get(self, request, task_id):
        task_result = generate_nutrition_landing_task.AsyncResult(task_id)
        if task_result.ready():
            result = task_result.result
            if result['status'] == 'completed':
                return Response(result['result'], status=status.HTTP_200_OK)
...
from api.tasks import (
    process_call_event,
    process_chat_message_notification,
    process_friend_request_notification,
    process_accept_friend_request_notification,
    process_comment_event,
    process_appointment_set_event,
    schedule_status_deletion
)

######################################################################################################################
class SendCallNotificationView(APIView):
    permission_classes = [IsAuthenticated]

    def post(self, request):
        data = request.data
        if not all(key in data for key in ["callerID", "receiverID", "sessionID", "type_of_call"]):
            return Response({"error": "Missing required data"}, status=status.HTTP_400_BAD_REQUEST)

        # Pass data to Celery task
        process_call_event.delay(data)
...
class SendChatNotificationView(APIView):
    def post(self, request):
        data = request.data
        required_fields = ["chat_id", "message_id", "sender_uid", "receiver_id", "message_body"]

        if not all(field in data for field in required_fields):
            return Response({"error": "Missing required data"}, status=status.HTTP_400_BAD_REQUEST)

        # Pass data to Celery task
        process_chat_message_notification.delay(data)

        return Response({"status": "queued"}, status=status.HTTP_202_ACCEPTED)


class FriendRequestNotificationView(APIView):
    permission_classes = [IsAuthenticated]
    def post(self, request):
        data = request.data

        # Ensure the required keys are provided
        ...
        # Pass data to Celery task
        process_friend_request_notification.delay(data)

        return Response({"status": "queued"}, status=status.HTTP_202_ACCEPTED)


class AcceptFriendRequestNotificationView(APIView):
    permission_classes = [IsAuthenticated]
    def post(self, request):
        data = request.data
        ...
        # Pass data to Celery task
        process_accept_friend_request_notification.delay(data)
...
class CommentEventView(APIView):
    ...
    def post(self, request):
        ...
        required_fields = ["post_id", "comment_id"]

        if not all(field in data for field in required_fields):
            return Response({"error": "Missing required data"}, status=status.HTTP_400_BAD_REQUEST)

        process_comment_event.delay(data)
        return Response({"status": "queued"}, status=status.HTTP_202_ACCEPTED)


class AppointmentSetEventView(APIView):
    permission_classes = [IsAuthenticated]
    def post(self, request):
        data = request.data

        required_fields = ["booker", "bookee", "time", "date"]

        if not all(field in data for field in required_fields):
            return Response({"error": "Missing required data"}, status=status.HTTP_400_BAD_REQUEST)

        process_appointment_set_event.delay(data)
        return Response({"status": "queued"}, status=status.HTTP_202_ACCEPTED)


class PostStatusEventView(APIView):
    permission_classes = [IsAuthenticated]
    def post(self, request):
        data = request.data
        ...
        schedule_status_deletion.delay(data)
        return Response({"status": "queued"}, status=status.HTTP_202_ACCEPTED)