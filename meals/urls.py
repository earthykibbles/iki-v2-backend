...
from meals.views import OnboardingView, UserProfileView, DailyMealPlanView, UserMealPlansView, NutritionPlanView, \
    RegisterView, ProcessImageView, NutritionPlanStatusView, DailyMealPlanStatusView
...
urlpatterns = [
    ...
    path('plan/status/<str:task_id>/', NutritionPlanStatusView.as_view(), name='nutrition_plan_status'),
    path('meal-plan/', DailyMealPlanView.as_view(), name='daily_meal_plan'),
    path('meal-plan/status/<str:task_id>/', DailyMealPlanStatusView.as_view(), name='daily_meal_plan_status'),
